...
type FonteDados = "demo" | "historico" | "oficial";

type HistoricoTseResponse = {
  sucesso: boolean;
  fonte: "historico-tse";
  ano: number;
  turno: number;
...
export default function Home() {
  ...
  const [fonteDados, setFonteDados] =
    useState<FonteDados>("demo");

  const [turnoHistorico, setTurnoHistorico] =
    useState(1);
  ...
  async function carregarResultados() {
    try {
      ...
      if (fonteDados === "historico") {
        ...
        const response = await fetch(
          `/api/historico-tse?${params.toString()}`,
          {
            cache:
              "no-store",
          }
        ...
        if (!response.ok) {
          throw new Error(
            `Falha ao consultar histórico TSE (${response.status}).`
          );
        }

        const data: HistoricoTseResponse =
          await response.json();

        if (!data.sucesso) {
          throw new Error(
            data.erro ??
              "Erro ao carregar histórico TSE."
          );
        }
      ...
      if (fonteDados === "oficial") {
        setCandidates([]);
        setTotalVotos(0);
        setTotalSecoes(0);
        setSecoesProcessadas(0);
        setPercentualTotalizacao(0);
        setLastUpdate(new Date());
        setError(
          "O modo Resultado oficial está reservado para a fonte oficial do TSE e ainda não possui dados conectados neste ambiente."
        );
        return;
      }
      ...
      if (ufSelecionada !== "Brasil") {
        params.set("uf", ufSelecionada);
      }

      if (municipioSelecionado) {
        params.set("municipio", municipioSelecionado);
      }

      if (zonaSelecionada) {
        params.set("zona", zonaSelecionada);
      }

      if (secaoSelecionada) {
        params.set("secao", secaoSelecionada);
      }
      ...
      setTotalSecoes(data.totalSecoes ?? 0);
      setSecoesProcessadas(
        data.secoesProcessadas ?? 0
      );
      setPercentualTotalizacao(
        data.percentualTotalizacao ?? 0
      );
      setLastUpdate(new Date());
    } catch (err) {
      console.error(
  ...
  function handleFonteChange(value: FonteDados) {
    setFonteDados(value);
    setUfSelecionada("Brasil");
    setMunicipioSelecionado("");
    setZonaSelecionada("");
  ...
  const fonteDetalhe =
    fonteDados === "historico"
      ? `TSE · Eleições Gerais 2022 · ${turnoHistorico}º turno`
      : fonteDados === "oficial"
        ? "Fonte oficial do TSE"
        : "Ambiente controlado de demonstração";
  ...
  return (
    <main className="shell">
          ...
          fonteDados === "historico"
            ...
            : fonteDados === "oficial"
              ...
              : tseLoading
                ...
                : tseStatus?.eleicao2026?.encontrada
                  ? "source-banner source-banner-official"
                  : "source-banner source-banner-demo"
        }
      >
        ...
        <div className="source-banner-content">
          ...
          <p>
            ...
            {fonteDados === "historico"
              ? `Dados históricos consolidados do TSE para as Eleições Gerais 2022. ${turnoHistorico}º turno.`
              : fonteDados === "oficial"
                ? "A conexão com a fonte oficial está separada da demonstração e do histórico TSE."
                : tseLoading
      ...
      <section className="hero">
        ...
        <div className="scope-box">
          ...
          <select
            ...
            onChange={(event) => {
              const value =
                event.target.value as FonteDados;
          ...
          {!loading &&
            ...
            candidates.length >
              ...
              <div className="candidate-list">
                ...
                {fonteDados === "demo" && secaoSelecionada && (
        ...
        <article className="card ai-card">
          ...
          <p className="ai-text">
            ...
            {topCandidate ? (
              <>
                {fonteDados === "demo"
                  ? "Neste ambiente de demonstração, o candidato"
                  : fonteDados === "historico"
                    ? "No histórico consolidado do TSE, o candidato"
                    : "No painel oficial, o candidato"} {" "}
                <strong>
                  {topCandidate.numero}
                </strong>{" "}
      ...
      <footer>
        ...
        <span>
          {fonteDados === "demo"
            ? "Demonstração · dados sintéticos separados das fontes eleitorais."
            : fonteDados === "historico"
              ? `Histórico TSE · 2022 · ${turnoHistorico}º turno.`
              : "Resultado oficial · integração oficial ainda não conectada."}
        </span>
